...
async def parse_command(channel_read, channel_write, context, args):
    ...
    if context.author is not None:  # Schedule is exempt from permission check
        if not access.authorization(cmd_data["command_permission"]["S"], context.author):
            return

    if context.author is not None:  # Schedule is exempt from cooldown check
    ...
    if cmd_data["command_type"]["S"] != "DYNAMIC":
        await channel_write.send(
            helper.parse_variables(cmd_data["command_response"]["S"], context.author, args)
        )
    else:
        # DYNAMIC
